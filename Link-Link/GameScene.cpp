 ...
 */
GameScene::GameScene(Scene *scene_last_, int m, int n, int totalkind,
                     int level_, bool gravity)
    : scene_last{scene_last_}, level{level_} {
 ...
 *
 */
GameScene::~GameScene() { delete map; }
...
void GameScene::onMouse(Sint32 x, Sint32 y) {
    ...
    if (map->getReady() && linearMousePositionOnMap >= 0 &&
        map->map[linearMousePositionOnMap]->getValid()) {
        ...
        } else if (map->isMatch(last, map->map[linearMousePositionOnMap]) ==
                   false) {
            /*如果无法匹配，则切换加框的图标*/
        ...
        //  printf("%d %d\n", map->map[linearMousePositionOnMap]->getX(),
        //  map->map[linearMousePositionOnMap]->getY());
    }
...
int GameScene::getMousePositionOnMap(Sint32 x, Sint32 y) {
    ...
    if ((x >= MAP_START_POSOTION_X) && (y >= MAP_START_POSOTION_Y) &&
        (x <= MAP_END_POSOTION_X) && (y <= MAP_END_POSOTION_Y)) {
...
bool GameScene::autoPlay() {
    ...
    if (!isWin) {
        map->isMatch(map->getFirstMatchedPair().first,
                     map->getFirstMatchedPair().second);
    }
